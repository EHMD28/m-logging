...
// !WARNING: this function should not be called directly. Instead, use
// corresponding macro
// * Newline is included as part of function.
static void mlog_log(const char* msg) { printf("[LOG]: %s\n", msg); }
...
/* testing */
static int mlog_test(const char* tag, int cond) {
    if (cond) {
...
static int mlog_testc(const char* color, const char* tag, int cond) {
    if (cond) {
